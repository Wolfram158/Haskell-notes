...
mergeSort [x] = ([x], 0)
mergeSort [x, y] = merge [x] [y] 0
mergeSort list = let half = floor ((fromIntegral (length list)) / 2)
...
countInversions list = snd (mergeSort list)

main = do
          putStrLn $ show $ mergeSort [1, 2, 3, 11, 4, 5, 6, 7]
      {-  putStrLn $ show $ mergeSort [6, -1, 3, 14, 9, 3, 2, -3, -100, 10, 15, 23, 11]   -}
          putStrLn $ show $ mergeSort [1, 8, 5, 3, 7, 2, 6, 4, 9]
